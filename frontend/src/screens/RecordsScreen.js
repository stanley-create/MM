...
export default function RecordsScreen({ navigation }) {
  ...
  }, [amount, category, description, navigation, showToast]);

  const toPreview = useCallback(() => {
    // Validate amount
    const amountValidation = validateAmount(amount, 0, 1000000);
    ...
    if (!descValidation.valid) {
      showToast(descValidation.error);
      return;
    }
    ...
    setPreviewData({
      ...
      carbon_est: (amountValidation.value * 0.01).toFixed(2),
      exp_est: 35,
    });
    setStage(1);
  }, [amount, category, description, showToast]);

  const renderInputStage = useCallback(
    () => (
      <View style={styles.stageContainer}>
        ...
        <View style={styles.row}>
          <TouchableOpacity
            ...
            onPress={() => Alert.alert("Cam", "Open OCR")}
          >
  ...
  const renderPreviewStage = useCallback(
    () => (
      <View style={styles.stageContainer}>
        ...
        <View style={styles.previewCard}>
          ...
          <Text style={styles.previewCat}>
            {previewData.category} - {description || "No Desc"}
          </Text>
          ...
          <View style={styles.statRow}>
            ...
            <Text style={{ fontWeight: "bold", color: COLORS.error }}>
              {previewData.carbon_est} kg
            </Text>
          </View>
          <View style={styles.statRow}>
            <Text>✨ Est. EXP:</Text>
            <Text style={{ fontWeight: "bold", color: COLORS.accent }}>
              +{previewData.exp_est} EXP
            </Text>
          </View>
        </View>
    ...
    [previewData, description, handleConfirm, loading, setStage],
  );
