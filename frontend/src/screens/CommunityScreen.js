...
export default function CommunityScreen() {
    const [messages, setMessages] = useState([
        { id: '1', user: 'ElfRanger', text: 'Just saved 5kg Carbon by walking!', type: 'chat' },
        { id: '2', user: 'MageAccountant', text: 'Great job! +50 EXP bonus sent.', type: 'chat' }
    ]);
    const [inputText, setInputText] = useState('');

    const handleSend = () => {
        if (!inputText.trim()) return;
        setMessages([...messages, { id: Date.now().toString(), user: 'Adventurer (You)', text: inputText, type: 'chat' }]);
        setInputText('');
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={[styles.msgContainer, item.user.includes('(You)') ? styles.myMsg : styles.theirMsg]}>
...
const styles = StyleSheet.create({
    ...
    msgContainer: { marginVertical: 5, padding: 10, borderRadius: 10, maxWidth: '80%' },
    ...
    sendBtn: { justifyContent: 'center', alignItems: 'center', backgroundColor: '#228B22', borderRadius: 20, paddingHorizontal: 20 },
    joinBtn: { margin: 10, backgroundColor: '#4169E1', padding: 15, borderRadius: 10, alignItems: 'center' },
