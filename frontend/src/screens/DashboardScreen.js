...
}; // Cute Elf/Mage placeholder

export default function DashboardScreen({ navigation }) {
  const [stats, setStats] = useState({ level: 5, exp: 450, carbon: 12.5 });
  const [npcMessage, setNpcMessage] = useState(
    "Welcome back, traveler! The forest looks greener today.",
  );
  const [lowPowerMode, setLowPowerMode] = useState(false);

  // Memoize calculated values to prevent re-renders
  const expPercentage = useMemo(() => {
    return `${(stats.exp / 1000) * 100}%`;
  }, [stats.exp]);

  // Sanitize NPC message to prevent XSS
  const safeNpcMessage = useMemo(() => sanitizeInput(npcMessage), [npcMessage]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.npcContainer}>
        ...
        <View style={styles.dialogBubble}>
          <Text style={styles.dialogText}>{safeNpcMessage}</Text>
      ...
      <View style={styles.statsCard}>
        ...
        <Text style={styles.predictText}>
          Prediction: Low spending this week!
        </Text>
      </View>
    </View>
  );
